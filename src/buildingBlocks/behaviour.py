        ...
        for goal in self._manager.goals:
            for (sensorName, indicator) in goal.wishes.iteritems():
                if sensorName in self._correlations.keys() and self._correlations[sensorName] * indicator > 0: # This means we affect the sensor in a way that is desirable by the goal
                    numBehavioursActivatedBySameGoal = len([b for b in self._manager.behaviours if sensorName in b.correlations.keys() and b.correlations[sensorName] * indicator > 0.0]) # the variable name says it all
                    rospy.logdebug("Calculating activation from goals for %s. There is/are %d behaviour(s) that supports %s via %s: %s",  self.name, numBehavioursActivatedBySameGoal, goal.name, sensorName, [b for b in self._manager.behaviours if sensorName in b.correlations.keys() and b.correlations[sensorName] * indicator > 0.0])
        ...
        for goal in self._manager.goals:
            for (sensorName, indicator) in goal.wishes.iteritems():
                if sensorName in self._correlations.keys() and self._correlations[sensorName] * indicator < 0: # This means we affect the sensor in a way that is not desirable by the goal
                    numBehavioursInhibitedBySameGoal = len([b for b in self._manager.behaviours if sensorName in b.correlations.keys() and b.correlations[sensorName] * indicator < 0.0]) # the variable name says it all
                    rospy.logdebug("Calculating inhibition from goals for %s. There is/are %d behaviour(s) that contradict %s via %s: %s",  self.name, numBehavioursInhibitedBySameGoal, goal.name, sensorName, [b for b in self._manager.behaviours if sensorName in b.correlations.keys() and b.correlations[sensorName] * indicator < 0.0])
        ...
        for behaviour in self._manager.behaviours:
            ...
            for (sensorName, indicator) in self._wishes.iteritems():
                if sensorName in behaviour.correlations.keys() and behaviour.correlations[sensorName] *  behaviour.preconditionSatisfaction * indicator > 0.0:  # If a predecessor can satisfy my precondition
                    numSuccessorsOfThatPredecessorAffectedByThisCorrelation = len([b for b in self._manager.behaviours if sensorName in b.wishes.keys() and b.wishes[sensorName] * indicator > 0.0]) # the variable name says it all
                    rospy.logdebug("Calculating activation from predecessors for %s. There is/are %d successor(s) of %s via %s: %s",  self._name, numSuccessorsOfThatPredecessorAffectedByThisCorrelation, behaviour.name, sensorName, [b for b in self._manager.behaviours if sensorName in b.wishes.keys() and b.wishes[sensorName] * indicator > 0.0])
            ...
            for (sensorName, indicator) in self._correlations.iteritems(): # this is what we do to sensors
                if sensorName in behaviour.wishes.keys():                  # if we affect other behaviours wishes somehow
                    if behaviour.wishes[sensorName] * indicator > 0:       # if we are a predecessor so we get activation from that successor
                        numPredecessorsOfThatSuccessorAffectedByThisCorrelation = len([b for b in self._manager.behaviours if sensorName in b.correlations.keys() and b.correlations[sensorName] * indicator > 0.0]) # the variable name says it all
                        rospy.logdebug("Calculating activation from successors for %s. There is/are %d predecessor(s) of %s via %s: %s", self._name, numPredecessorsOfThatSuccessorAffectedByThisCorrelation, behaviour.name, sensorName, [b for b in self._manager.behaviours if sensorName in b.correlations.keys() and b.correlations[sensorName] * indicator > 0.0])
            ...
            for (sensorName, indicator) in self._correlations.iteritems(): # this is what we do to sensors
                if sensorName in behaviour.wishes.keys():                  # if we affect other behaviours somehow
                    if behaviour.wishes[sensorName] * indicator < 0:       # if we are a conflictor, we get inhibited
                        numBehavioursThatConflictWithThatBehaviourBecauseOfTheSameCorrelation = len([b for b in self._manager.behaviours if sensorName in b.correlations.keys() and b.correlations[sensorName] * behaviour.wishes[sensorName] < 0.0]) # the variable name says it all
                        rospy.logdebug("Calculating inhibition from conflicted for %s. %s is conflicted via %s by %d behaviour(s): %s", self.name, behaviour.name, sensorName, numBehavioursThatConflictWithThatBehaviourBecauseOfTheSameCorrelation, [b for b in self._manager.behaviours if sensorName in b.correlations.keys() and b.correlations[sensorName] * behaviour.wishes[sensorName] < 0.0])
