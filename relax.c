...
static int *lch_E = NULL;
static int lnum_ch_E = 0;

static int *l0P_E = NULL;
static int lnum_0P_E = 0;
...
void collect_1P_and_A_info( void ){
  ...
  if ( relax_collect_1P_and_A_info ) {
    gA = ( int * ) calloc( gnum_op_conn, sizeof( int ) );
    gnum_A = 0;
    relax_collect_1P_and_A_info = FALSE;
  }
...
void collect_A_info( void ){
  ...
  if ( relax_collect_A_info ) {
    gA = ( int * ) calloc( gnum_op_conn, sizeof( int ) );
    gnum_A = 0;
    relax_collect_A_info = FALSE;
  }
...
void collect_A_axioms_info( void ){
  ...
  if ( relax_collect_A_axioms_info ) {
    gA_axioms = ( int * ) calloc( gnum_op_conn, sizeof( int ) );
    gnum_A_axioms = 0;
    relax_collect_A_axioms_info = FALSE;
  }
...
void initialize_fixpoint( State *S ){
  ...
  if ( relax_initialize_fixpoint ) {
    ...
    relax_initialize_fixpoint = FALSE;
  }
...
void collect_H_info( void ){
  ...
  if ( relax_collect_H_info ) {
    gH = ( int * ) calloc( gnum_op_conn, sizeof( int ) );
    gnum_H = 0;
    relax_collect_H_info = FALSE;
  }
