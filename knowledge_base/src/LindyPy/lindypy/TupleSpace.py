...
class Manager(object):
    ...
    def __next__(self):
        ...
        while self.registered and not msg:
            ...
            (ready, _, _) = select.select(list(pipes.keys()), [], [], 1)
            for p in ready:
                pid = pipes[p]
            ...
            for pid in list(self.registered.keys()):
                if pid not in pids:
    ...
    def handle_interests(self, pid, tup):
        ...
        """
        res = []
        for (ipid, pat) in list(self.consuming_int.items()):
            res.append((ipid, True, pat))
        for (ipid, pat) in list(self.non_consuming_int.items()):
