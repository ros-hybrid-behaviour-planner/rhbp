...

{
  ...
  /* TODO: we have a memory leak here but the data structure contains cycles so I'm stuck in an endless loop when I try to free it.
  if(lehc_space_head != NULL)
      free_EhcNode(lehc_space_head);*/
...
Bool search_for_better_state( State *S, int h, State *S_, int *h_ ){
  ...
  if ( search_search_for_better_state ) {
    make_state( &S__, gnum_ft_conn, gnum_fl_conn );
    search_search_for_better_state = FALSE;
  }
...
int expand_first_node( int h ){
  ...
  if ( search_expand_first_node ) {
    make_state( &S_, gnum_ft_conn, gnum_fl_conn );
    search_expand_first_node = FALSE;
  }
...
BfsNode* do_best_first_search( void )
...
BfsNode* do_weighted_Astar( void )
...

{
  ...
  for ( i = lbfs_space_head->next; i; i = i->next ) {
     ...
     */
    if ( min_goal_distance == -1 || i->goal_distance < min_goal_distance ) {
...
void add_to_Astar_epsilon_space( State *S, int op, BfsNode *father ){
    ...
    if ( search_add_to_Astar_epsilon_space ) {
        remaining = (int*) calloc(gnum_op_conn, sizeof(int));
        search_add_to_Astar_epsilon_space = FALSE;
    }
...
Bool extract_plan( BfsNode *last ){
  ...
  if ( search_extract_plan ) {
      if(ops != NULL)
          free(ops);
    ops = ( int * ) calloc(MAX_PLAN_LENGTH, sizeof(int));
    search_extract_plan = FALSE;
  }
...
void do_axiom_update( State *dest ){
    ...
    if ( search_do_axiom_update ) {
        in_dest = ( Bool * ) calloc( gnum_ft_conn, sizeof( Bool ) );
        for ( i = 0; i < gnum_ft_conn; i++ ) {
            in_dest[i] = FALSE;
        }
        search_do_axiom_update = FALSE;
    }
