...
void free_Facts( Facts *source ){
	if ( source ) {
	    free_Facts(source->next);
	    free(source->fact);
	    free( source );
	}
}

void free_FluentValues( FluentValues *source ){
	if ( source ) {
		free_FluentValues(source->next);
		free( source );
	}
}
...
void free_MixedOperator( MixedOperator *o ){
    int i = 0;
    if(o){
        if(o->next)
            free_MixedOperator(o->next);
        if(o->preconds)
            free(o->preconds);
        if ( o->numeric_preconds_comp ) {
            free( o->numeric_preconds_comp );
        ...
        if(o->effects)
            free_partial_Effect(o->effects); /* Not sure if "partial" is the right 'destructor' here ... */
        free(o);
    }
}
...
void free_EhcNode(EhcNode* n){
    if(n){
        free_EhcNode(n->father);
        n->father = NULL;
        free_EhcNode(n->next);
        n->next = NULL;
    }
