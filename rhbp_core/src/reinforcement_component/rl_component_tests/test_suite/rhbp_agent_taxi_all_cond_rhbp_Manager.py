...
from test_environment import RewardSensor, BehaviorShell, TestTaxiEnv, LocationCondition


class TaxiAgentManagerAllRHBP(RhbpAgentBase):
    def __init__(self):
        super(TaxiAgentManagerAllRHBP, self).__init__()
    ...
    def init_behaviors(self):
        ...
        self.state_sensor.rl_extension = RlExtension(state_space=500, encoding=EncodingConstants.HOT_STATE)
        ...
        # conditions
        is_at_pickup_location = LocationCondition(sensor=self.passenger_state_sensor, activator=ThresholdActivator(4),
                                                  sensors=state_list, is_pick_up=True)
        ...
        row_one_cond = Condition(self.destination_state_sensor, ThresholdActivator(0))
        col_one_cond = Condition(self.destination_state_sensor, ThresholdActivator(0))
        pass_one_cond = Condition(self.destination_state_sensor, ThresholdActivator(0))  # to get the dest in the input
        ...
        is_in_goal_state = Condition(self.state_sensor, ThresholdActivator(thresholdValue=15,
                                                                           name="state"))  # for getting the state of the game

        good_pick_off = Condition(reward_sensor,
                                  ThresholdActivator(thresholdValue=20, name="good"))  # successful dropoff
        illegal_action = Condition(reward_sensor, ThresholdActivator(thresholdValue=-10, isMinimum=False,
                                                                     name="illegal"))  # wrong action
