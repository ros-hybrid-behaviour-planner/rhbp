...
class TaxiAgent(RhbpAgentBase):
    ...
    def init_behaviors(self):
        ...
        action_two_behavior = MakeActionBehavior(plannerPrefix=self.prefix, name="ActionOne",reward_sensor=reward_sensor,
                                                 action_index=1,state_sensor=self.state_sensor, environment=self.env)

        action_three_behavior = MakeActionBehavior(plannerPrefix=self.prefix, name="ActionTwo",reward_sensor=reward_sensor,
                                                 action_index=2,state_sensor=self.state_sensor, environment=self.env)

        action_four_behavior = MakeActionBehavior(plannerPrefix=self.prefix, name="ActionThree",reward_sensor=reward_sensor,
                                                 action_index=3,state_sensor=self.state_sensor, environment=self.env)

        action_five_behavior = MakeActionBehavior(plannerPrefix=self.prefix, name="ActionFour",reward_sensor=reward_sensor,
                                                 action_index=4,state_sensor=self.state_sensor, environment=self.env)
        action_six_behavior = MakeActionBehavior(plannerPrefix=self.prefix, name="ActionFive",reward_sensor=reward_sensor,
                                                 action_index=5,state_sensor=self.state_sensor, environment=self.env)

        is_in_goal_state = Condition(self.state_sensor, ThresholdActivator(thresholdValue=15))  # for getting the state of the game
