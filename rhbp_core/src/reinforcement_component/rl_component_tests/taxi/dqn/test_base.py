    ...
    def start_env(self, num_prints=100, threshold=10, random_seed=0, should_print=True):
        ...
        for i in range(1, self.max_episodes):
            ...
            if self.cycles_last == num_prints:
                ...
                if should_print:
                    print(self.counter, self.rewards_all / float(self.counter_last), self.rewards, i,
                          (self.rewards / i) * 100, self.rewards_last / float(num_prints))
                # if last reward is over threshold print time after stop process
                if self.rewards_last / float(num_prints) > threshold or self.counter > 3000000:
                    print(self.__class__.__name__, "Over threshold", threshold, "after", self.counter, "steps and", i,
                          "episodes")
