    ...
    def add_trial(self):
        ...
        while not is_end:
            ...
            if is_shoot:
                ...
                # shoot the ball
                shoot_speed = 70
                dist_ball, dist_goal, is_shoot = self.get_tuple_go_to_ball_shoot(dist_ball, dist_goal + 0.5, 0,
                                                                                 num_go_to_first_ball, shoot_speed)
                ...
                for i in range(0, int(num_go_to_ball_shoot)):  # gototheball ater the shot
                    dist_ball, dist_goal, shoot_speed = self.get_tuple_go_to_ball_shoot(dist_ball, dist_goal, i + 1,
                                                                                        num_go_to_first_ball,
                                                                                        shoot_speed)
                    tuple = [1, 1, 1, dist_ball, dist_goal, shoot_speed]
                    experience = [old_tuple, tuple, 5, step_reward]
                ...
                # dribble the ball
                dribble_speed = 40
                dist_ball, dist_goal, dribble_speed = self.get_tuple_go_to_ball_dribble(dist_ball, dist_goal + 0.5, 0,
                                                                                        num_go_to_first_ball,
                                                                                        dribble_speed)
                ...
                for i in range(0, num_go_to_ball_dribble):  # gototheball ater the shot
                    dist_ball, dist_goal, dribble_speed = self.get_tuple_go_to_ball_dribble(dist_ball, dist_goal, i + 1,
                                                                                            num_go_to_first_ball,
                                                                                            dribble_speed)
                    tuple = [1, 1, 1, dist_ball, dist_goal, dribble_speed]
                    experience = [old_tuple, tuple, 5, step_reward]
